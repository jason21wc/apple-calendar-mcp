...
| Events with other people on them | Removing one requires your explicit confirmation, and the prompt says what is actually true: that the event will be removed through EventKit, that this **may notify the other participants**, and that the attendee list **cannot be put back**. EventKit exposes no notification-suppression control and no explicit RSVP setter; exact removal behavior is unmeasured — so the server calls the operation *remove*, and does not pretend it is the same as clicking Decline in Calendar.app |
