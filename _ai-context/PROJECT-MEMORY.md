...
| ~~C2a~~ — *(superseded: Desktop asks on every call under `toolPolicy: ask`, which has no always-allow)* Always confirm, even on a single match | 2026-08-17 | No auto-delete when the search returns exactly one candidate. A single match is precisely where skipping confirmation is most tempting and where injection would aim. |
...
| **C7 — Every mutation must be restorable** | 2026-08-20 | **NEW, and it replaces C2/C2a/C4a as the primary control.** "Restore" means *the information is back*, NOT that the original object returns — a new event carrying the same title, times, notes, location and recurrence satisfies it. That reframing, from the human, dissolves what killed three delete designs: identifier stability, invitation state and series membership are all unsolvable, and all irrelevant once identity is not the requirement. **The C6 boundary and the restorability boundary turn out to be the same line**: attendees are the one field a snapshot cannot reproduce (readonly in EventKit), and C6 already refuses those events. So everything this tool may delete, it can fully put back. |
...
| 67 | **The model will wave itself through an ESCALATE when it believes it has standing authorization** — observed, not theorised: the agent proceeded past an ESCALATE verdict on a delete because the human had asked for it a turn earlier. Reasonable in that instance, and fatal as a control. It is the empirical case for `toolPolicy` being the only approval mechanism this project counts, since it is enforced in the host process where the model cannot reach it. | 2026-08-20 |
...
| Requirement | Status |
|---|---|
| Reading must **never** ask for permission | **Met.** Read tools carry `readOnlyHint: true` and do not prompt |
| Create, change and delete must be approved — **delete at minimum** | Design settled: per-tool `toolPolicy: "ask"`, which offers Allow-once/Deny with no persistent always-allow |
