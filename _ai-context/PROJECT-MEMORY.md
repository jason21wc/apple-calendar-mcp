...
| 41 | Top-level-code globals in `main.swift` (`disclaimMode`, `stateDir`, `store`) are `@MainActor`-isolated and **SEGV the test host** when read from a test, killing the whole run. This makes `Doctor.run()` and `SetupFlow.run()` untestable in-process. Fix is to move them into a type with lazy statics. | 2026-08-19 |
